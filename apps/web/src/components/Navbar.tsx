...
import { Sparkles, Menu, X, Download, BookOpen } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 glass border-b border-slate-800/80">
      ...
      {mobileOpen && (
        <div className="md:hidden glass border-b border-slate-800 px-4 pt-3 pb-6 space-y-4">
          <Link href="/#features" onClick={() => setMobileOpen(false)} className="block text-base font-medium text-slate-300 hover:text-white">Features</Link>
          <Link href="/#daily-challenge" onClick={() => setMobileOpen(false)} className="block text-base font-medium text-slate-300 hover:text-white">Daily Challenge</Link>
          <Link href="/#ai-mentor" onClick={() => setMobileOpen(false)} className="block text-base font-medium text-slate-300 hover:text-white">AI Mentor</Link>
          <Link href="/#study-tools" onClick={() => setMobileOpen(false)} className="block text-base font-medium text-slate-300 hover:text-white">Study Tools</Link>
          ...
          <div className="pt-2">
            <Link href="/#download" onClick={() => setMobileOpen(false)} className="w-full flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-pink-600 text-white font-semibold shadow-lg shadow-indigo-500/25">
              <Download className="w-4 h-4" /> Download App Free
            </Link>
          </div>
        </div>
      )}
